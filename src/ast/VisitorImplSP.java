...
public class VisitorImplSP extends SimplePlusBaseVisitor<ElementBase> {
	...
	public StmtAssignment visitAssignment(AssignmentContext ctx) {
		return new StmtAssignment(ctx.ID().getText(),(Exp) visit(ctx.exp()), ctx.start.getLine(), ctx.start.getCharPositionInLine());
	}
	
	@Override
	public StmtDelete visitDeletion(DeletionContext ctx) {
		return new StmtDelete(ctx.ID().getText(), ctx.ID().getSymbol().getLine(), ctx.ID().getSymbol().getCharPositionInLine());
	}
	...
	public StmtRet visitRet(RetContext ctx) {
		return new StmtRet(ctx.exp() == null ? null : (Exp) visit(ctx.exp()), lastArgsDimension, ctx.start.getLine(), ctx.start.getCharPositionInLine());
	}
	
	@Override
	public StmtIte visitIte(IteContext ctx) {
		return new StmtIte((Exp) visit(ctx.exp()), visitStatement(ctx.statement(0)), ctx.statement(1) == null ? null : visitStatement(ctx.statement(1)), ctx.start.getLine(), ctx.start.getCharPositionInLine());
	}
	
	@Override
	public StmtCall visitCall(CallContext ctx) {
		List<Exp> exps = ctx.exp() == null ? Collections.emptyList() : ctx.exp().stream().map(p->(Exp)visit(p)).collect(Collectors.toList());
	...
	public Exp visitBinExp(BinExpContext ctx) {
		...
		switch (ctx.op.getText()) {
			case "+": return new ExpSum(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "-": return new ExpDiff(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "*": return new ExpMult(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "/": return new ExpDiv(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "<": return new ExpLessThan(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "<=": return new ExpLessThanEq(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case ">": return new ExpGreaterThan(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case ">=": return new ExpGreaterThanEq(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "==": return new ExpEqual(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "!=": return new ExpNotEqual(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "&&": return new ExpAnd(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
			case "||": return new ExpOr(left, right, ctx.start.getLine(), ctx.start.getCharPositionInLine());
	...
	public ExpVal visitValExp(ValExpContext ctx) {
		return new ExpVal(Integer.parseInt(ctx.NUMBER().getText()), ctx.start.getLine(), ctx.start.getCharPositionInLine());
	}
